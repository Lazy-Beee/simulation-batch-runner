...
2. Edit `config.json` for your machine:
   ...
   - `simulator.priority` (optional, Windows) — kernel scheduling priority class for the simulator process: `"idle"`, `"below_normal"`, `"normal"` (default), `"above_normal"`, `"high"`, or `"realtime"`. Use `below_normal`/`idle` to keep the machine responsive while a batch runs in the background; `high` to favor the sim over other work. Avoid `realtime` (can starve OS threads). Unknown values fall back to `normal`. Under MPI the class is set on `mpiexec` and inherited by the ranks; non-Windows platforms ignore it.
   - `upload` (optional) — post-zip upload of each case archive to a cloud remote via [rclone](https://rclone.org/). Runs after a successful zip and before cleanup; the local archive is kept unless a case's **Cleanup** is set to `Both` (and only then once the upload has actually succeeded — see the Queue workflow below).
     ...
     - `async` (optional, TUI only) — `true` (default) uploads on a dedicated background worker, separate from zip, so a transfer overlaps zipping the next case (network vs disk); uploads are still serialised one at a time. `false` uploads inline, blocking the zip step until the transfer finishes. Independent of `simulator.zip_async`.
...
- (TUI only) Archiving and uploading run on two independent background workers, each serial (one 7z / one rclone at a time, so parallel cases don't thrash disk or bandwidth) but concurrent with each other — an upload of one case overlaps zipping the next. `simulator.zip_async` (default `true`) gates the zip stage; `upload.async` (default `true`) gates the upload stage; set either to `false` to run that stage inline and block. The batch drains both queues (zips then uploads) before declaring idle.
- Per case the pipeline is **zip → folder cleanup → upload → archive cleanup**. The folder removal (`folder` / `both`) happens right after a successful zip. The archive is then copied to the rclone remote (`rclone copy`) when `upload.enabled`; `both` deletes the local archive afterwards **only if the upload succeeded** — so a failed (or skipped) upload always leaves the archive on disk to retry. A failed zip cancels all cleanup.
